...
def WriteTrueExpression(SimuCount, TransLength, OUTfile):
	fp=open(OUTfile, 'w')
	fp.write("# Name\tLength\tCopyNumber\tNumReads\n")
	for t,v in SimuCount.items():
		assert(t in TransLength)
		truecount = v
		length = TransLength[t]
		fp.write("{}\t{}\t{}\t{}\n".format(t, length, 1.0*truecount/length, truecount))
	fp.close()


if __name__=="__main__":
	if len(sys.argv)==1:
		print("python3 GroupTruthExp.py <target_sequence> <sample_01_1.fasta.gz> <outputprefix>")
	else:
		TargetseqFile=sys.argv[1]
		FastaGZFile=sys.argv[2]
		OUTprefix=sys.argv[3]

		Simulated, TransLength = ReadSimulatedStartPos(TargetseqFile, FastaGZFile)
		#pickle.dump(Simulated, open(OUTprefix+"simupos.pickle", 'wb'))
		SimuCount = {t:np.sum(v) for t,v in Simulated.items()}
		for tname,l in TransLength.items():
			if not (tname in SimuCount):
				SimuCount[tname] = 0
		WriteTrueExpression(SimuCount, TransLength, OUTprefix+"expression_truth.txt")
